...
#+begin_src emacs-lisp
  (global-set-key (kbd "C-c w")
                  (lambda () (interactive) (find-file "~/org-brain/week-2021-01-25.org")))
#+end_src
* Org
...
#+begin_src emacs-lisp
  (setq org-todo-keywords
        '((sequence "TODO(t!)" "NEXT(n!)" "STARTED(s!)" "WAITING(w!)" "|" "DONE(d!)" "PASS(p!)")))
...
#+begin_src emacs-lisp
  (setq org-capture-templates
             '(("t" "Todo" entry (file+headline "~/org-brain/tasks.org" "Tasks")
                "* TODO %?\nAt: %U From: %a  \n %i\n")
               ("j" "Journal" entry (file+datetree "~/org-brain/journal.org")
                "* %?\nAt: %U From: %a \n")
               ("c" "Clocked In Task" plain
                (clock)
                "%i%?" :empty-lines 1)
               ))
#+end_src
