...
* Load
#+begin_src elisp
  (load-file "~/code/elisp/week-planviews.el")
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; Modeline... Doom not working (way too big and size is inconsistent)
...
#+begin_src elisp
  ...
  (global-set-key (kbd "C-c f c")
                  (lambda () (interactive) (my/file-shortcut "~/org/code.org")))
  (global-set-key (kbd "C-c f C")
                  (lambda () (interactive) (my/file-shortcut "~/org/career.org")))
  ...
  (global-set-key (kbd "C-c f p")
                  (lambda () (interactive) (my/file-shortcut "~/org/personal.org")))
  (global-set-key (kbd "C-c f w")
                  (lambda () (interactive) (my/file-shortcut (get-week-file))))
...
#+begin_src elisp
  ...
  (setq org-clock-in-switch-to-state #'my/switch-task-on-clock-start)

  (defun my/switch-task-on-clock-start (task-state)
  ...
  (setq org-agenda-span 5
        ...
        org-agenda-prefix-format " %i %-12:c%?-12t% s"  ;; Since I use tags over files
  ...
  (setq org-agenda-custom-commands
        '(
          ("h" "High priority"
           ((tags-todo "+PRIORITY=\"A\""
                       ((org-agenda-overriding-header "High Priority\n")
                        (org-agenda-sorting-strategy '(tag-up todo-state-up priority-down))
          ...
          ("c" "Computing Config"
           ((tags-todo "comp|emacs"
                       ((org-agenda-overriding-header "Computing Config\n")
                        (org-agenda-sorting-strategy '(tag-up todo-state-up priority-down))
                        ))))
  ...
  ;; Example from https://orgmode.org/worg/org-tutorials/org-custom-agenda-commands.html
  ;; (setq org-agenda-custom-commands
  ;;       '(("P" "Printed agenda"
...
** Tags, Capture, TODO
:PROPERTIES:
:ID:       50c19694-a173-4f8e-a562-a4ae3b35874c
:END:
#+begin_src emacs-lisp
  ;; Tags
  ...
  (setq org-capture-templates
        '(
          ...
          ("l" "Links" entry (file "~/org/links.org")
           "* %?\n:PROPERTIES:\n:CREATED: %U\n:END:\n"
           :prepend t)
          ("s" "Shelf" entry (file+headline "~/org/personal.org" "Shelf")
           "* TODO %?\n:PROPERTIES:\n:CREATED: %U\n:END:\n"
           :prepend t)
          ("z" "Zettle Inbox" entry (file "~/org/notes/inbox.org")
           "* %?\n:PROPERTIES:\n:CREATED: %U\n:END:\n"
           :prepend t)
          ))
  ...
  (setq org-todo-keywords
        '((sequence  "TODO(t)" "ACTIVE(a)" "LATER(l)" "WAIT(w)" "SOMEDAY(s)" "PROJECT(P)" "|" "PASS(p)" "DONE(d)")))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; THIS IS THE ROOT OF THE ISSUES with citar-select-ref
  (defun my/org-roam-node-from-cite (keys-entries)
      (interactive (list (citar-select-ref :multiple nil :rebuild-cache t)))
      (let ((title (citar--format-entry-no-widths (cdr keys-entries)
                                                  "${author year editor} :: ${title}")))
        (org-roam-capture- :templates
                           '(("s" "sournce" plain "%?" :if-new
                              (file+head "source/${citekey}.org"
                                         ":PROPERTIES:
  ...
  (use-package citar
    ...
    :custom
    (citar-bibliography '("~/org/notes/sources.bib")))
